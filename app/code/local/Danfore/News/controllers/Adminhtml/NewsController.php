 ...
 * Time: 13:44
 */
class Danfore_News_Adminhtml_NewsController extends Mage_Adminhtml_Controller_action
{
    ...
    public function editAction()
    {
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent(
                $this->getLayout()->createBlock('news/adminhtml_news_edit'))
                ->_addLeft($this->getLayout()->createBlock('news/adminhtml_news_edit_tabs'));
            $this->renderLayout();
        } else {
            Mage::getSingleton('adminhtml/session')
                ->addError(Mage::helper('news')->__('Item does not exist'));
            $this->_redirect('*/*/');
        }
    }
    ...
    public function saveAction()
    {
        ...
        try {
            ...
            $model->save();
            Mage::getSingleton('adminhtml/session')
                ->addSuccess(Mage::helper('news')->__('Item was successfully saved'));
            Mage::getSingleton('adminhtml/session')
                ->setFormData(false);
        ...
        Mage::getSingleton('adminhtml/session')
            ->addError(Mage::helper('news')->__('Unable to find item to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($newsIds)) {
            ...
            try {
                ...
                $news->delete();
                Mage::getSingleton('adminhtml/session')
                    ->addSuccess(Mage::helper('adminhtml')->__('Total of %d record(s) were successfully deleted', count($newsIds)));
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')
                    ->addError($e->getMessage());
            }
            ...
            try {
                foreach ($newsIds as $newsId) {
                    $news = Mage::getSingleton('news/news')
                        ...
                        ->setIsMassupdate(true)->save();
                }
                $this->_getSession()
                    ->addSuccess($this->__('Total of %d record(s) were  successfully updated',
                    count($newsIds)));
            } catch (Exception  $e) {
